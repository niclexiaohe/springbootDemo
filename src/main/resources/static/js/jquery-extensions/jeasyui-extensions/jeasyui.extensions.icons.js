...
(function ($, undefined) {
    ...
    $.easyui.icons.showSelector = function (options) {
        if (options && options.topMost && $ != $.util.$) {
            return $.util.$.easyui.icons.showSelector.apply(this, arguments);
        }
        var opts = $.extend({
            ...
            dia = $.easyui.showDialog($.extend({}, opts, {
                ...
                onSave: function () {
                    if ($.isFunction(opts.onEnter)) {
                        return opts.onEnter.call(this, value);
                    }
                }
            }));
        $.util.exec(function () {
            ...
            tabsOpts.onSelect = function (title, index) {
                ...
                if (tab.find("ul.icon-selector-ul").length) {
                    return;
                }
                var icons = $.array.filter($.easyui.icons[style], opts.size == "32" ?
                        function (icon) {
                            return icon.iconCls.indexOf("32") > -1;
                        } :
                        function (icon) {
                            return icon.iconCls.indexOf("32") == -1;
                        }
                ...
                if (opts.size == "32") {
                    ul.addClass("icon-selector-ul-32");
                }
            };
            $.util.exec(function () {
                dia.setValue(value);
            ...
            function refreshCheckedStatus() {
                ...
                if ($.isArray(value)) {
                    $.each(value, function (i, n) {
                        if (n) {
                            li.find("span." + n).parent().addClass("selected");
                        }
                    });
                } else {
                    if (value) {
                        last = li.find("span." + value).parent().addClass("selected");
                    }
                }
            };
        });
