...
(function ($, undefined) {
    ...
    function create(target) {
        ...
        if (opts.originalValue) {
            textarea.val(opts.originalValue);
        }

        if (!panelId) {
            panel.attr("id", panelId = state.panelId = "codemirror_panel_" + $.util.guid("N"));
        }
        ...
        panel.panel($.extend({}, opts, {
            onResize: function (width, height) {
                ...
                if ($.isFunction(opts.onResize)) {
                    opts.onResize.apply(target, arguments);
                }
            },
    ...
    function initialEvents(target) {
        ...
        $.each($.fn.codemirror.events, function (i, n) {
            ...
            state.cm.on(n, function () {
                ...
                if ($.isFunction(e)) {
                    return e.apply(target, arguments);
                }
            });
        });
    };
    ...
    function replaceRange(target, param) {
        if (!param || !param.replacement || !param.from || !param.to) {
            return;
        }
        var state = $.data(target, "codemirror");
    ...
    function findPosH(target, param) {
        if (!param || !param.start || !param.amount || !param.unit || !param.visually) {
            return;
        }
        var state = $.data(target, "codemirror");
    ...
    function findPosV(target, param) {
        if (!param || !param.start || !param.amount || !param.unit) {
            return;
        }
        var state = $.data(target, "codemirror");
    ...
    function on(target, param) {
        if (!param || !param.type || !param.func) {
            return;
        }
    ...
    function undo(target) {
        return $.data(target, "codemirror").doc.undo();
    };

    function redo(target) {
        return $.data(target, "codemirror").doc.redo();
    };

    function undoSelection(target) {
        return $.data(target, "codemirror").doc.undoSelection();
    };

    function redoSelection(target) {
        return $.data(target, "codemirror").doc.redoSelection();
    };

    function historySize(target) {
        return $.data(target, "codemirror").doc.historySize();
    };

    function clearHistory(target) {
        return $.data(target, "codemirror").doc.clearHistory();
    };

    function getHistory(target) {
        return $.data(target, "codemirror").doc.getHistory();
    };

    function setHistory(target, history) {
        return $.data(target, "codemirror").doc.setHistory(history);
    };
    ...
    function setGutterMarker(target, param) {
        if (!param || !param.line || !param.gutterID || !param.value) {
            return;
        }
    ...
    function addLineClass(target, param) {
        if (!param || !param.line || !param.where || !param.cls) {
            return;
        }
    ...
    function addWidget(target, param) {
        if (!param || !param.pos || !param.node || !param.scrollIntoView) {
            return;
        }
    ...
    function addLineWidget(target, param) {
        if (!param || !param.line || !param.node) {
            return;
        }
    ...
    function scrollIntoView(target, param) {
        if (!param || !param.what || !param.height) {
            return;
        }
    ...
    function cursorCoords(target, param) {
        if (!param || !param.where || !param.mode) {
            return;
        }
    ...
    $.fn.codemirror.methods = {
        ...
        panel: function (jq) {
            return $.data(jq[0], "codemirror").panel;
        },

        textarea: function (jq) {
            return $.data(jq[0], "codemirror").textarea;
        },

        cm: function (jq) {
            return $.data(jq[0], "codemirror").cm;
        },

        doc: function (jq) {
            return $.data(jq[0], "codemirror").doc;
        },
        ...
        //  
        getValue: function (jq, separator) {
            return getValue(jq[0], separator);
        },
        ...
        getRange: function (jq, param) {
            return getRange(jq[0], param);
        },

        // param: { replacement: string, from: {line, ch}, to: {line, ch}, ?origin: string }
        replaceRange: function (jq, param) {
            return jq.each(function () {
                replaceRange(this, param);
            });
        },

        getLine: function (jq, index) {
            return getLine(jq[0], index);
        },

        lineCount: function (jq) {
            return lineCount(jq[0]);
        },

        lastLine: function (jq) {
            return lastLine(jq[0]);
        },

        getLineHandle: function (jq, index) {
            return getLineHandle(jq[0], index);
        },

        getLineNumber: function (jq, handle) {
            return getLineNumber(jq[0], handle);
        },

        // param: function(line:LineHandler) | { start: integer, end: integer, f: (line: LineHandle) }
        eachLine: function (jq, param) {
            return jq.each(function () {
                eachLine(this, param);
            });
        },

        markClean: function (jq) {
            return jq.each(function () {
                markClean(this);
            });
        },

        changeGeneration: function (jq, closeEvent) {
            return changeGeneration(jq[0], closeEvent);
        },

        isClean: function (jq, generation) {
            return isClean(jq[0], generation);
        },

        getSelection: function (jq, lineSep) {
            return getSelection(jq[0], lineSep);
        },

        getSelections: function (jq, lineSep) {
            return getSelections(jq[0], lineSep);
        },

        // param: { replacement: string, ?select: string }
        replaceSelection: function (jq, param) {
            return jq.each(function () {
                replaceSelection(this, param);
            });
        },

        // param: { replacements: array<string>, ?select: string }
        replaceSelections: function (jq, param) {
            return jq.each(function () {
                replaceSelections(this, param);
            });
        },

        //  ?start: string
        //  return: {line, ch}
        getCursor: function (jq, start) {
            return getCursor(jq[0], start);
        },

        //  return: array<{ anchor, head }>
        listSelections: function (jq) {
            return listSelections(jq[0]);
        },

        somethingSelected: function (jq) {
            return somethingSelected(jq[0]);
        },

        //  param: { line, ch}|number, ?ch: number, ?options: object }
        setCursor: function (jq, param) {
            return jq.each(function () {
                setCursor(this, param);
            });
        },

        //  param: { anchor: {line, ch}, ?head: {line, ch}, ?options: object }
        setSelection: function (jq, param) {
            return jq.each(function () {
                setSelection(this, param);
            });
        },

        //  param: { ranges: array<{anchor, head}>, ?primary: integer, ?options: object }
        setSelections: function (jq, param) {
            return jq.each(function () {
                setSelections(this, param);
            });
        },

        //  param: { anchor: {line, ch}, ?head: {line, ch} }
        addSelection: function (jq, param) {
            return jq.each(function () {
                addSelection(this, param);
            });
        },

        //  param: { from: {line, ch}, ?to: {line, ch}, ?options: object }
        extendSelection: function (jq, param) {
            return jq.each(function () {
                extendSelection(this, param);
            });
        },

        //  param: { heads: array<{line, ch}>, ?options: object }
        extendSelections: function (jq, param) {
            return jq.each(function () {
                extendSelections(this, param);
            });
        },

        //  param: { f: function(range: {anchor, head}) → {anchor, head}), ?options: object }
        extendSelectionsBy: function (jq, param) {
            return jq.each(function () {
                extendSelectionsBy(this, param);
            });
        },

        setExtending: function (jq, value) {
            return jq.each(function () {
                setExtending(this, value);
            });
        },

        //  return: boolean
        getExtending: function (jq) {
            return getExtending(jq[0]);
        },

        //  return: boolean
        hasFocus: function (jq) {
            return hasFocus(jq[0]);
        },

        //  param: { start: {line, ch}, amount: integer, unit: string, visually: boolean }
        //  return: { line, ch, ?hitSide: boolean }
        findPosH: function (jq, param) {
            return findPosH(jq[0], param);
        },

        //  param: { start: {line, ch}, amount: integer, unit: string }
        //  return: { line, ch, ?hitSide: boolean }
        findPosV: function (jq, param) {
            return findPosV(jq[0], param);
        },
        ...
        setOption: function (jq, param) {
            return jq.each(function () {
                setOption(this, param);
            });
        },

        //  option: string
        //  return: any
        getOption: function (jq, option) {
            return getOption(jq[0], option);
        },

        //  param: { map: object, bottom: boolean }
        addKeyMap: function (jq, param) {
            return jq.each(function () {
                addKeyMap(this, param);
            });
        },

        //  map: object
        removeKeyMap: function (jq, map) {
            return jq.each(function () {
                removeKeyMap(this, map);
            });
        },

        // param: { mode: string|object, ?options: object }
        addOverlay: function (jq, param) {
            return jq.each(function () {
                addOverlay(this, param);
            });
        },

        //  mode: string|object
        removeOverlay: function (jq, mode) {
            return jq.each(function () {
                removeOverlay(this, mode);
            });
        },

        //  param: { type: string, func: (...args) }
        on: function (jq, param) {
            return jq.each(function () {
                on(this, param);
            });
        },
        ...
        //  doc: CodeMirror.Doc
        //  return: CodeMirror.Doc
        swapDoc: function (jq, doc) {
            return swapDoc(jq[0], doc);
        },

        //  copyHistory: boolean
        //  return: CodeMirror.Doc
        copy: function (jq, copyHistory) {
            return copy(jq[0], copyHistory);
        },

        //  param: { shareHist: boolean, from: integer, to: integer, mode: string|object }
        linkedDoc: function (jq, param) {
            return linkedDoc(jq[0], param);
        },

        //  doc: CodeMirror.Doc
        unlinkDoc: function (jq, doc) {
            return jq.each(function () {
                unlinkDoc(this, doc);
            });
        },
        ...
        iterLinkedDocs: function (jq, f) {
            return jq.each(function () {
                iterLinkedDocs(this, f);
            });
        },
        ...
        undoSelection: function (jq) {
            return jq.each(function () {
                undoSelection(this);
            });
        },

        redoSelection: function (jq) {
            return jq.each(function () {
                redoSelection(this);
            });
        },
        ...
        historySize: function (jq) {
            return historySize(jq[0]);
        },

        clearHistory: function (jq) {
            return jq.each(function () {
                clearHistory(this);
            });
        },
        ...
        //  return: object
        getHistory: function (jq) {
            return getHistory(jq[0]);
        },

        //  history: object
        setHistory: function (jq, history) {
            return jq.each(function () {
        ...
        //      } }
        //  return: TextMarker
        markText: function (jq, param) {
            return markText(jq[0], param);
        },

        //  param: { pos: {line, ch}, ?options: { widget: Element, insertLeft: boolean, shared: boolean } }
        //  return: TextMarker
        setBookmark: function (jq, param) {
            return setBookmark(jq[0], param);
        },

        //  param:  { from: {line, ch}, to: {line, ch} }
        //  return: array<TextMarker>
        findMarks: function (jq, param) {
            return findMarks(jq[0], param);
        },

        //  pos:    { line, ch }
        //  return: array<TextMarker>
        findMarksAt: function (jq, pos) {
            return findMarksAt(jq[0], pos);
        },

        //  return: array<TextMarker>
        getAllMarks: function (jq) {
            return getAllMarks(jq[0]);
        },


        //  param:  { line: integer|LineHandle, gutterID: string, value: Element }
        //  return: LineHandle
        setGutterMarker: function (jq, param) {
            return setGutterMarker(jq[0], param);
        },

        //  gutterID: string
        clearGutter: function (jq, gutterID) {
            return jq.each(function () {
                clearGutter(this, gutterID);
            });
        },

        //  param:  { line: integer|LineHandle, where: string, cls: string }
        //  return: LineHandle
        addLineClass: function (jq, param) {
            return addLineClass(jq[0], param);
        },

        //  param:  { line: integer|LineHandle, where: string, cls: string }
        //  return: LineHandle
        removeLineClass: function (jq, param) {
            return removeLineClass(jq[0], param);
        },

        //  line:   integer|LineHandle
        //  return: object
        lineInfo: function (jq, line) {
            return lineInfo(jq[0], line);
        },

        //  param:  { pos: {line, ch}, node: Element, scrollIntoView: boolean }
        addWidget: function (jq, param) {
            return jq.each(function () {
                addWidget(this, param);
            });
        },
        ...
        addLineWidget: function (jq, param) {
            return addLineWidget(jq[0], param);
        },
        ...
        getScrollInfo: function (jq) {
            return getScrollInfo(jq[0]);
        },

        //  param:  { what: {line, ch}|{left, top, right, bottom}|{from, to}|null, ?margin: number }
        scrollIntoView: function (jq, param) {
            return jq.each(function () {
                scrollIntoView(this, param);
            });
        },

        //  param:  { where: boolean|{line, ch}, mode: string }
        //  return: { left, top, bottom }
        cursorCoords: function (jq, param) {
            return cursorCoords(jq[0], param);
        },

        //  param:  { pos: {line, ch}, ?mode: string }
        //  return: { left, right, top, bottom }
        charCoords: function (jq, param) {
            return charCoords(jq[0], param);
        },

        //  param:  { object: {left, top}, ?mode: string }
        //  return: { line, ch }
        coordsChar: function (jq, param) {
            return coordsChar(jq[0], param);
        },

        //  param:  { height: number, ?mode: string }
        //  return: number
        lineAtHeight: function (jq, param) {
            return lineAtHeight(jq[0], param);
        },

        //  param:  { line: number, ?mode: string }
        //  return: number
        heightAtLine: function (jq, param) {
            return heightAtLine(jq[0], param);
        },

        //  return: number
        defaultTextHeight: function (jq) {
            return defaultTextHeight(jq[0]);
        },

        //  return: number
        defaultCharWidth: function (jq) {
            return defaultCharWidth(jq[0]);
        },

        //  return: { from: number, to: number }
        getViewport: function (jq) {
            return getViewport(jq[0]);
        },


        //  return: object
        getMode: function (jq) {
            return $.data(jq[0], "codemirror").doc.getMode();
        },

        //  pos: { line, ch }
        //  return: object
        getModeAt: function (jq, param) {
            return $.data(jq[0], "codemirror").doc.getModeAt(param);
        },

        //  param: { pos: {line, ch}, ?precise: boolean }
        //  return: { start, end, string, type, state }
        getTokenAt: function (jq, param) {
            return getTokenAt(jq[0], param);
        },

        //  param: { pos: { line, ch } }
        //  return: string
        getTokenTypeAt: function (jq, pos) {
            return $.data(jq[0], "codemirror").cm.getTokenTypeAt(pos);
        },

        //  param:  { pos: {line, ch}, type: string }
        //  return: array<helper>
        getHelpers: function (jq, param) {
            return getHelpers(jq[0], param);
        },

        //  param: { pos: {line, ch}, type: string }
        //  return: helper
        getHelper: function (jq, param) {
            return getHelper(jq[0], param);
        },

        //  param: { ?line: integer, ?precise: boolean }
        //  return: object
        getStateAfter: function (jq, param) {
            return getStateAfter(jq[0], param);
        },


        //  func: func: () → any
        //  return: any
        operation: function (jq, func) {
            return $.data(jq[0], "codemirror").cm.operation(func);
        },

        //  param:  { line: integer, ?dir: string|integer }
        indentLine: function (jq, param) {
            return jq.each(function () {
                indentLine(this, param);
            });
        },

        //  ?value: boolean
        toggleOverwrite: function (jq, value) {
            return jq.each(function () {
                toggleOverwrite(this, value);
            });
        },

        //  name: string
        execCommand: function (jq, name) {
            return jq.each(function () {
                execCommand(this, name);
            });
        },

        //  index: integer
        //  return: { line, ch }
        posFromIndex: function (jq, index) {
            return posFromIndex(jq[0], index);
        },

        //  param:  { line, ch }
        //  return : integer
        indexFromPos: function (jq, param) {
            return indexFromPos(jq[0], param);
        },

        //  return: TextAreaElement
        getInputField: function (jq) {
            return getInputField(jq[0]);
        },

        //  return: Element
        getWrapperElement: function (jq) {
            return getWrapperElement(jq[0]);
        },

        //  return: Element
        getScrollerElement: function (jq) {
            return getScrollerElement(jq[0]);
        },

        //  return: Element
        getGutterElement: function (jq) {
            return getGutterElement(jq[0]);
        },
