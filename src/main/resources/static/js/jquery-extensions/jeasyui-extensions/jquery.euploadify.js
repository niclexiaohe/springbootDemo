...
(function ($, undefined) {
    ...
    function refreshQueueStatus(queues, file, value) {
        var q = $.array.first(queues, function (f) {
            return f.id == file.id;
        })
        if (q) {
    ...
    function initializeUploadify(target) {
        ...
        var uopts = {
            ...
            onCancel: function (file) {
                $.array.remove(state.queues, file, function (q) {
                    return file.id == q.id;
                });
            ...
            onDialogClose: function () {
                return opts.onDialogClose.apply(target, arguments);
            },
            onDialogOpen: function () {
                return opts.onDialogOpen.apply(target, arguments);
            },
            onDisable: function () {
                return opts.onDisable.apply(target, arguments);
            },
            onEnable: function () {
                return opts.onEnable.apply(target, arguments);
            },
            onFallback: function () {
                return opts.onFallback.apply(target, arguments);
            },
            onInit: function () {
                return opts.onInit.apply(target, arguments);
            },
            onQueueComplete: function () {
                return opts.onQueueComplete.apply(target, arguments);
            },
            ...
            onSelectError: function () {
                return opts.onSelectError.apply(target, arguments);
            },
            onSWFReady: function () {
                return opts.onSWFReady.apply(target, arguments);
            },
            ...
            onUploadSuccess: function (file, data, response) {
                ...
                var values = opts.multi ? $.array.map(getQueuesByStatus(target, -4), function (f) {
                    return f.value;
                }) : [val];
        ...
        state.uploadButton = $("<a class=\"euploadify-button euploadify-upload" + (opts.auto && !opts.multi ? " euploadify-hidden" : "") + "\"></a>").appendTo(state.buttonbar).linkbutton({
            ...
            onClick: function () {
                var queues = getQueues(target),
                    ff = $.array.first(queues, function (val) {
                        return val.filestatus != -4;
                    });
        ...
        if (opts.multi && template == "grid") {
            ...
            state.removeButton = $("<a class=\"euploadify-button\"></a>").appendTo(state.buttonbar).linkbutton({
                ...
                onClick: function () {
                    ...
                    if (!rows || !rows.length) {
                        return $.easyui.messager.show(opts.message.mustNotEmpty);
                    }
                    $.each(array, function (i, row) {
                        var index = state.grid.datagrid("getRowIndex", row);
                        if (index > -1) {
                            state.grid.datagrid("deleteRow", index);
                        }
                        state.uploadify.uploadify("cancel", row.id);
                        $.array.remove(state.queues, row, function (q) {
                            return row.id == q.id;
                        });
                    });
                    var values = $.array.map(getQueuesByStatus(target, -4), function (f) {
                        return f.value;
                    });
                    setValues(target, values);
                }
        ...
        if (opts.tooltip) {
            ...
            if (state.button) {
                state.button.tooltip({ content: tooltip.button });
            }
            if (state.uploadButton) {
                state.uploadButton.tooltip({ content: tooltip.upload });
            }
            if (state.stopButton) {
                state.stopButton.tooltip({ content: tooltip.stop });
            }
            if (state.cancelButton) {
                state.cancelButton.tooltip({ content: tooltip.cancel });
            }
            if (state.removeButton) {
                state.removeButton.tooltip({ content: tooltip.remove });
            }
        }
    };
    ...
    function buildMultiPanel(target, uopts) {
        ...
        state.queueDOM.on("click", ".uploadify-queue-item .cancel a", function () {
            ...
            $.array.remove(state.queues, id, function (q) {
                return q.id == id;
            });
            var queues = getQueuesByStatus(target, -4),
                values = $.array.map(queues, function (q) {
                    return q.value;
                });
            ...
            rowContextMenu: [
                {
                    ...
                    disabled: function () {
                        return opts.disabled || state.wrapper.is(".euploadify-disabled");
                    },
    ...
    function getFileName(target, file) {
        if (!file || !file.name) {
            return $(target).euploadify("options").emptyText;
        }
    ...
    function getQueuesByStatus(target, status) {
        return $.array.filter(getQueues(target), function (q) {
            return q.filestatus == status;
        });
    };
    ...
    function disable(target, setDisabled) {
        ...
        if (setDisabled) {
            ...
            if (opts.buttonPlain) {
                state.button.addClass("l-btn-plain-disabled");
            }
            ...
            if (opts.buttonPlain) {
                state.button.removeClass("l-btn-plain-disabled");
            }
    ...
    function getValues(target) {
        var state = $.data(target, "euploadify"), opts = state.options,
            ...
            values = $.array.map(fields, function (val) {
                return $(val).val();
            });
        ...
        var opts = $.extend({}, $.easyui.showEuploadifyDialog.defaults, options || {}),
            ...
            dia = $.easyui.showDialog($.extend({}, opts, {
                ...
                onBeforeDestroy: function () {
                    if (euploadify) {
                        euploadify.euploadify("destroy");
                    }
                },
                onSave: function () {
                    return opts.onEnter.call(dia, getValue(), euploadify, uploadify, this);
                },
                onClose: function () {
                    return opts.onCancel.call(dia, getValue(), euploadify, uploadify, this);
                }
            }));
        $.util.exec(function () {
    ...
    $.fn.euploadify.methods = {
        ...
        panel: function (jq) {
            return $.data(jq[0], "euploadify").panel;
        },

        uploadify: function (jq) {
            return getUploadify(jq[0]);
        },
    ...
    $.extend($.fn.euploadify.defaults, {
        ...
        requestFilter: function (data, file) {
            return data;
        },
        responseFilter: function (data, file) {
            return $.util.tryExec(function () {
