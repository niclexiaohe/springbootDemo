...
(function ($, undefined) {
    ...
    function resetCache(iframe) {
        var array = $.array.filter(cache, function (val) {
            return val.current == iframe;
        }), l = array.length;
        while (l--) {
            $.array.remove(cache, array[l]);
        }
        ;
    };
    ...
    var getParent = function () {
        ...
        var p = $.array.first(cache, function (val) {
    ...
    $.easyui._showDialog = function (opts, currentFrame) {
        if (opts.onApply == null || opts.onApply == undefined) {
            opts.onApply = opts.onSave;
        }
        if (opts.onSave == null || opts.onSave == undefined) {
            opts.onSave = opts.onApply;
        }
        ...
        if (!$.util.likeArray(opts.toolbar)) {
            opts.toolbar = [];
        }
        ...
        if (!$.util.likeArrayNotString(opts.buttons)) {
            opts.buttons = [];
        }
        ...
        if (opts.buttonsPlain) {
            bottombuttons.linkbutton("setPlain", true);
        }
        if (!opts.iniframe) {
            if (opts.href) {
    ...
    function parseExtensionsEnd(target) {
        ...
        opts.iniframe = exts.iniframe;
        opts.bodyCls = exts.bodyCls;
        opts.bodyStyle = exts.bodyStyle;
        if (cp && cp.length) {
            if (opts.bodyCls) {
                cp.addClass(opts.bodyCls);
            }
            if (opts.bodyStyle) {
                cp.css(opts.bodyStyle);
            }
        }
        if (opts.iniframe) {
            refresh(target);
