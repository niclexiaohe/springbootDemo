...
(function ($, undefined) {
        ...
        append: function (button, init) {
            ...
            editorui[ci] = function (cmd) {
                return function (editor) {
                    ...
                    if ($.isFunction(b.onInit)) {
                        $.util.exec(function () {
                            b.onInit.call(ui, editor);
                        });
                    }
                    return ui;
                };
            ...
            function createDialog() {
                ...
                $.util.exec(function () {
                    ...
                    fontset.fontfamily = $("<input class=\"ueditor-fontset-fontfamily\" type=\"text\" />").appendTo(cc).combobox({
                        ...
                        onSelect: function (r) {
                            editor.execCommand("fontfamily", r.text);
                        },
                        onShowPanel: function () {
                            fontset.fontsize.combobox("hidePanel");
                        }
                    });
                    fontset.fontsize = $("<input class=\"ueditor-fontset-fontsize\" type=\"text\" />").appendTo(cc).combobox({
                        ...
                        onSelect: function (r) {
                            editor.execCommand("fontsize", r.text);
                        },
                        onShowPanel: function () {
                            fontset.fontfamily.combobox("hidePanel");
                        }
                    });
            ...
            if (editor.fontset && !editor.fontset.dialog("options").closed) {
                if (editor.fontset.fontfamily && fontfamily) {
                    ...
                    $.each(array, function (i, family) {
                        ...
                        family = $.trim(family);
                        if ($.array.contains(data, family, function (r) {
                            return r.value == family;
                        })) {
                    ...
                    if (!b) {
                        editor.fontset.fontfamily.combobox("setValue", "arial");
                    }
                }
    ...
    $.easyui.showUeditorDialog = function (options) {
        ...
        var opts = $.extend({}, $.easyui.showUeditorDialog.defaults, options || {}),
            ...
            dia = $.easyui.showDialog($.extend({}, opts, {
                ...
                onDestroy: function () {
                    if (ueditor) {
                        ueditor.ueditor("destroy");
                    }
                },
                onResize: function () {
                    var ret = $.fn.dialog.defaults.onResize.apply(this, arguments);
                    if ($.isFunction(opts.onResize)) {
                        opts.onResize.apply(this, arguments);
                    }
                    return ret;
                },
                onSave: function () {
                    return opts.onEnter.call(dia, value, ueditor, editor, this);
                },
                onClose: function () {
                    if (editor && editor.fontset) {
                        editor.fontset.dialog("close");
                    }
        ...
        $.util.exec(function () {
            var container = dia.find(".ueditor-dialog-container").ueditor($.extend({}, opts, {
                ...
                oncontentChange: function () {
                    ...
                    if ($.isFunction(opts.oncontentChange)) {
                        opts.oncontentChange.apply(this, arguments);
                    }
                }
            }));
    ...
    $.fn.ueditor.methods = {
        ...
        panel: function (jq) {
            return $.data(jq[0], "ueditor").panel;
        },
        ...
        textarea: function (jq) {
            return $.data(jq[0], "ueditor").textarea;
        },
        ...
        addshortcutkey: function (jq, param) {
            return jq.each(function () {
                return jq.each(function () {
                    addshortcutkey(this, param);
                });
            });
        },

        //  获取编辑器的内容。 可以通过参数定义编辑器内置的判空规则
        getContent: function (jq, filter) {
            return getContent(jq[0], filter);
        },

        //  取得完整的html代码，可以直接显示成完整的html文档
        getAllHtml: function (jq) {
            return getAllHtml(jq[0]);
        },
        ...
        execCommand: function (jq, cmdName) {
            return execCommand(jq[0], cmdName);
        },

        //  根据传入的command命令，查选编辑器当前的选区，根据命令返回相关的值
        queryCommandState: function (jq, cmdName) {
            return queryCommandState(jq[0], cmdName);
        },

        //  根据传入的command命令，查选编辑器当前的选区，根据命令返回相关的值
        queryCommandValue: function (jq, cmdName) {
            return queryCommandValue(jq[0], cmdName);
        },

        //  检查编辑区域中是否有内容，若包含参数tags中的节点类型，直接返回true
        hasContents: function (jq, tags) {
            return hasContents(jq[0], tags);
        },
        ...
        setEnabled: function (jq, except) {
            return jq.each(function () {
                setEditorDisabled(this, false, except);
            });
        },

        //  设置当前编辑区域不可编辑,except中的命令除外
        setDisabled: function (jq, except) {
            return jq.each(function () {
                setEditorDisabled(this, true, except);
            });
        },
        ...
        //  根据指定的路径，获取对应的语言资源
        getLang: function (jq, path) {
            return getLang(jq[0], path);
        },

        //  计算编辑器html内容或纯文本字符串的长度
        getContentLength: function (jq, ingoneHtml) {
            return getContentLength(jq[0], ingoneHtml);
        },

        //  注册输入过滤规则
        addInputRule: function (jq, rule) {
            return jq.each(function () {
                addInputRule(this, rule);
            });
        },

        //  注册输出过滤规则
        addOutputRule: function (jq, rule) {
            return jq.each(function () {
                addOutputRule(this, rule);
            });
        },
