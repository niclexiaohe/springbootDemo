...
(function ($, undefined) {
        ...
        var state = $.data(target, "comboselector"), opts = state.options,
            ...
            t = $(target).addClass("comboselector-f").combo($.extend({}, opts, {
                onShowPanel: function () {
                        ...
                        onEnter: function (val) {
                            if (val) {
                                var isArray = $.util.likeArrayNotString(val),
                                    values = isArray ? $.array.map(val, function (value) {
                    ...
                    if ($.isFunction(opts.onShowPanel)) {
                        opts.onShowPanel.apply(this, arguments);
                    }
                },
                onHidePanel: function () {
                    ...
                    if ($.isFunction(opts.onHidePanel)) {
                        opts.onHidePanel.apply(this, arguments);
                    }
                },
                onDestroy: function () {
                    if (state.dialog) {
                    ...
                    if ($.isFunction(opts.onDestroy)) {
                        opts.onDestroy.apply(this, arguments);
                    }
                },
    ...
    function setValues(target, values) {
        var t = $(target), state = $.data(target, "comboselector"), opts = state.options,
            ...
            getText = function (value) {
                var row = $.array.first(state.data, function (val) {
                    return val[state.idField] == value;
                });
                return row ? row[opts.textField] : null;
            },
            hasTextField = $.util.likeArrayNotString(state.data) && opts.textField,
            text = hasTextField ? $.array.map(array, function (val) {
                return getText(val);
            }).join(opts.separator) : $.array.join(array, opts.separator);
    ...
    $.fn.comboselector.methods = {
        ...
        getData: function (jq) {
            return $.data(jq[0], "comboselector").data;
        },
