...
(function ($, undefined) {
    ...
    var updateRow = function (target, param) {
        if (!param || param.id == undefined || !param.row) {
            return;
        }
        var t = $(target), opts = t.treegrid("options");
        if ($.isFunction(opts.onBeforeUpdate) && opts.onBeforeUpdate.call(target, param.id, param.row) == false) {
            return;
        }
        ...
        if ($.isFunction(opts.onUpdate)) {
            opts.onUpdate.call(target, param.id, param.row);
        }
    };
    var appendRow = function (target, param) {
        ...
        var t = $(target), opts = t.treegrid("options");
        if ($.isFunction(opts.onBeforeAppend) && opts.onBeforeAppend.call(target, param.parent, param.data) == false) {
            return;
        }
        ...
        $.each(param.data, function () {
            initColumnRowTooltip(t, opts, this[opts.idField], this);
        });
        if ($.isFunction(opts.onAppend)) {
            opts.onAppend.call(target, param.parent, param.data);
        }
    };
    var insertRow = function (target, param) {
        if (!param || !param.data || (!param.before && !param.after)) {
            return;
        }
        var t = $(target), opts = t.treegrid("options");
        if ($.isFunction(opts.onBeforeInsert) && opts.onBeforeInsert.call(target, param.before, param.after, param.data) == false) {
            return;
        }
        ...
        if ($.isFunction(opts.onInsert)) {
            opts.onInsert.call(target, param.before, param.after, param.data);
        }
    };
    var resetTeeIndent = function (t, opts, id, field) {
        ...
        $.each(child, function () {
            resetTeeIndent(t, opts, this[opts.idField], field);
        });
    };
    ...
    var freezeColumn = function (target, field) {
        ...
        if (!frozenFields || !frozenFields.length || !$.array.contains(fields, field) || $.array.contains(frozenFields, field)) {
            return;
        }
    ...
    var unfreezeColumn = function (target, field) {
        var t = $(target), fields = t.treegrid("getColumnFields"), frozenFields = t.treegrid("getColumnFields", true);
        if (!fields || !fields.length || $.array.contains(fields, field) || !$.array.contains(frozenFields, field)) {
            return;
        }
    ...
    var moveColumn = function (target, param) {
        if (!param || !param.source || !param.target || param.source == param.target || !param.point) {
            return;
        }
        ;
        ...
        var t = $(target);
        if (t.treegrid("hasMuliRowHeader")) {
            return;
        }
        ...
        if (sourceFrozen == undefined && $.array.contains(frozenFields, param.source)) {
            sourceFrozen = true;
        }
        if ($.array.contains(fields, param.target)) {
            targetFrozen = false;
        }
        if (targetFrozen == undefined && $.array.contains(frozenFields, param.target)) {
            targetFrozen = true;
        }
        if (sourceFrozen == undefined || targetFrozen == undefined) {
            return;
        }
        if ($.isFunction(opts.onBeforeMoveColumn) && opts.onBeforeMoveColumn.call(target, param.source, param.target, param.point) == false) {
            return;
        }
        ...
        if (index > -1) {
            $.array.insert(targetColumns, param.point == "before" ? index : index + 1, sourceOpts);
        }
        ...
        if (sourceFrozen || targetFrozen && sourceFrozen != targetFrozen) {
            var data = t.treegrid("getData");
            t.treegrid("loadData", data);
        } else {
            t.treegrid("fixColumnSize");
        }
        if (sourceFrozen) {
            if (!targetFrozen) {
        ...
        if ($.isFunction(opts.onMoveColumn)) {
            opts.onMoveColumn.call(target, param.source, param.target, param.point);
        }
    }
    ...
    var shiftColumn = function (target, param) {
        ...
        if (index == -1 || (param.point == "before" && index == 0) || (param.point == "after" && index == fields.length - 1)) {
            return;
        }
    ...
    var isChecked = function (target, id) {
        ...
        return $.array.contains(rows, id, function (val) {
            return val[opts.idField] == id;
        });
    };
    ...
    var moveRow = function (target, param) {
        ...
        if (!$.array.contains(["append", "top", "bottom"], param.point)) {
            param.point = "append";
        }
        var t = $(target), opts = t.treegrid("options"),
        ...
        if ($.isFunction(opts.onBeforeDrop) && opts.onBeforeDrop.call(target, targetNode, sourceNode, param.point) == false) {
            return;
        }
        if (t.treegrid("isParent", { id1: param.source, id2: param.target })) {
            return;
        }
        var node = t.treegrid("pop", param.source);
        switch (param.point) {
            case "append":
                t.treegrid("append", { parent: param.target, data: [node] });
                break;
            case "top":
                t.treegrid("insert", { before: param.target, data: node });
                break;
            case "bottom":
                t.treegrid("insert", { after: param.target, data: node });
                break;
            default:
                t.treegrid("append", { parent: param.target, data: [node] });
                break;
        ...
        if (node && $.isFunction(opts.onDrop)) {
            opts.onDrop.call(target, targetNode, sourceNode, param.point);
        }
    };

    var shiftRow = function (target, param) {
        if (!param || !param.id || !param.point || !$.array.contains(["up", "upLevel", "down", "downLevel"], param.point)) {
            return;
        }
        var t = $(target), opts = t.treegrid("options"), node;
        switch (param.point) {
            case "up":
                node = t.treegrid("prevRow", param.id);
                break;
            case "upLevel":
                node = t.treegrid("getParent", param.id);
                break;
            case "down":
                node = t.treegrid("nextRow", param.id);
                break;
            case "downLevel":
                node = t.treegrid("prevRow", param.id);
                break;
    ...
    var getNearChildren = function (target, id) {
        ...
        return $.array.filter(children, function (val) {
            return t.treegrid("getParent", val[opts.idField])[opts.idField] == id;
        });
    };
    ...
    var enableRowDnd = function (target) {
        ...
        t.treegrid("getPanel").find("div.datagrid-view div.datagrid-body table tr.datagrid-row").draggable({
            ...
            proxy: function (source) {
                ...
                if (td.length > 6) {
                    $("<td>...</td>").css("width", "40px").appendTo(temp);
                }
                ...
                if ($.isFunction(opts.onBeforeDrag) && opts.onBeforeDrag.call(target, row) == false) {
                    return false;
                }
                if (e.which != 1) {
                    return false;
            ...
            onDragLeave: function (e, source) {
                ...
                if ($.isFunction(opts.onDragLeave)) {
                    opts.onDragLeave.call(target, droperRow, dragerRow);
                }
            },
    ...
    var getRowDom = function (target, param) {
        ...
        if (cascade) {
            ...
            $.each(children, function (i, n) {
                var d = getRowDom(target, n[opts.idField]);
                dom = dom.add(d);
            });
        }
        return dom;
    ...
    var getCellDom = function (target, pos) {
        if (!pos || !pos.field || pos.id == null || pos.id == undefined) {
            return $();
        }
    ...
    var showRow = function (target, param, grid, options, extensions, refreshable) {
        ...
        if (dom.length) {
            ...
            $.each(children, function () {
                $.array.remove(exts.filterData, this);
            });
            if (refreshable) {
    ...
    var showRows = function (target, param) {
        ...
        if (param === true) {
            ...
            array = $.array.map(array, function (val) {
                return val[opts.idField];
            });
        } else if ($.array.likeArray(param) && !$.util.isString(param)) {
            array = param;
        } else {
        ...
        if (array) {
            $.each(array, function (index, val) {
                showRow(target, val, t, opts, exts, false);
            });
    ...
    var hideRows = function (target, param) {
        ...
        if (array) {
            $.each(array, function (index, val) {
                hideRow(target, val, t, opts, exts, false);
            });
            refreshColumnFilterStatus(t, opts, exts, t.treegrid("getRows"));
        }
    };
    ...
    var getHiddenRows = function (target, cascade) {
        ...
        return $.array.filter(exts.filterData, function (val) {
            return $.array.contains(roots, val);
        });
    };
    ...
    var getVisibleRows = function (target, cascade) {
        ...
        return $.array.filter(rows, function (val) {
            return $.array.contains(exts.filterData, val) ? false : true;
        });
    };
        ...
        if ($.isFunction(param)) {
            ...
            $.each(function () {
                ...
                if (node != null && node != undefined) {
                    t.treegrid("remove", this[opts.idField]);
                }
            });
        }
    };
    ...
    var selectRow = function (target, param) {
        ...
        if (cascade) {
            ...
            $.each(t.treegrid("getChildren", id), function () {
                _select.call(t, t, this[opts.idField]);
            });
        }
    };
    ...
    var unselectRow = function (target, param) {
        ...
        if (cascade) {
            var opts = t.treegrid("options");
            $.each(t.treegrid("getChildren", id), function () {
                _unselect.call(t, t, this[opts.idField]);
            });
        }
    };
    ...
    var checkRow = function (target, param) {
        ...
        if (cascade) {
            var opts = t.treegrid("options");
            $.each(t.treegrid("getChildren", id), function () {
                t.datagrid("checkRow", this[opts.idField]);
            });
        }
    };
    ...
    var uncheckRow = function (target, param) {
        ...
        if (cascade) {
            var opts = t.treegrid("options");
            $.each(t.treegrid("getChildren", id), function () {
                t.datagrid("uncheckRow", this[opts.idField]);
            });
        }
    };
    ...
    var columnFilterSelect = function (target, param) {
        ...
        if ($.util.isBoolean(param)) {
            t.treegrid(param ? "showRows" : "hideRows", true);
            return;
        }
    ...
    var livesearch = function (target, param) {
        ...
        cells.each(function () {
            ...
            if (field == treeField) {
                cell = cell.find("span.tree-title");
            }
    ...
    function initHeaderColumnFilterContainer(t, opts, exts) {
        ...
        headerFields.each(function () {
            ...
            if (field) {
                initColumnFilterField(t, opts, exts, container, colOpts, rows, headerFields);
            }
        });
    ...
    function initColumnFilterField(t, opts, exts, container, colOpts, rows, headerFields) {
        ...
        switch (filter) {
            case "checkbox":
                initColumnFilterFieldCheckBox(t, opts, exts, container, field, rows, distinctVals);
                break;
            case "livebox":
                initColumnFilterFieldLiveBox(t, opts, container, field, rows);
                break;
            case "caps":
            ...
            var item = $("<div></div>").addClass("datagrid-header-filter-item").attr("text", text).appendTo(container),
                ...
                handler = function () {
                    var filterRows = $.array.filter(rows, function (value) {
                        ...
                        hiddenRows = $.array.filter(filterData, function (value) {
                            return value[field] == text;
                        });
                    t.treegrid(hiddenRows.length ? "showRows" : "hideRows", $.array.map(filterRows, function (val) {
                        return val[opts.idField];
                    }));
                };
            item.click(handler);
    ...
    function initColumnFilterFieldLiveBox(t, opts, container, field, rows) {
        ...
        var btn = $("<a />").linkbutton({ plain: true, iconCls: "icon-search" }).appendTo(container).click(function () {
            ...
            t.treegrid("hideRows", $.array.map(filterRows, function (val) {
                return val[opts.idField];
            }));
            input.focus();
        });
    ...
    function changeSliderValue(t, opts, field, rows, value, type, input, slider, headerFileds) {
        ...
        t.treegrid("showRows", true).treegrid("hideRows", $.array.map(filterRows, function (val) {
            return val[opts.idField];
        }));
    ...
    function refreshColumnFilterCheckbox(t, exts, rows, td, field) {
        td.find("div.datagrid-header-filter-item").each(function () {
            ...
            var length = $.array.sum(rows, function (val) {
                ...
                filterData = t.treegrid("getHiddenRows"),
                hiddenLength = $.array.sum(filterData, function (val) {
                    return val[field] == text ? 1 : 0;
                }),
    ...
    function setItemIconCls(icon, iconCls) {
        icon.removeClass("tree-checkbox0 tree-checkbox1 tree-checkbox2").addClass(iconCls);
    };
    ...
    function initRowContextMenu(t, opts, exts) {
        ...
        opts.onContextMenu = function (e, row) {
            if ($.isFunction(exts.onContextMenuBak)) {
                exts.onContextMenuBak.apply(this, arguments);
            }
    ...
    function parseHeaderBaseContextMenuItems(t, opts, exts, e, field, eventData) {
        ...
        if (typeof exp == "object") {
            exp = $.extend({ current: false, all: false, submenu: true }, exp);
        }
        var m1 = {
            text: "升序", iconCls: "icon-standard-hmenu-asc", disabled: sortable != true,
            handler: function () {
                return t.treegrid("sort", { sortName: field, sortOrder: "asc" });
            }
        };
        var m2 = {
            text: "降序", iconCls: "icon-standard-hmenu-desc", disabled: sortable != true,
            handler: function () {
                return t.treegrid("sort", { sortName: field, sortOrder: "desc" });
            }
        };
        var m3 = {
            text: "显示/隐藏列", iconCls: "icon-standard-application-view-columns", disabled: false, children: [
                {
                    ...
                    var count = $.array.sum(exts.fieldOptions, function (val) {
                ...
                },
                {
                    ...
                    $.each(exts.fieldOptionsBackup, function () {
                        t.treegrid(this.hidden == true ? "hideColumn" : "showColumn", this.field);
                    });
                    var mm = $(this).parent();
                    mm.children("div.menu-item:gt(1)").each(function () {
                        var title = $(this).text(), colOpts = $.array.first(exts.fieldOptions, function (val) {
                            return val.title == title;
                        });
                        if (colOpts) {
                            menu.menu("setIcon", { target: this, iconCls: colOpts.hidden ? "tree-checkbox0" : "tree-checkbox1" });
                        }
                        menu.menu("enableItem", this);
                    });
        ...
        var m5 = { text: "导出当前页", iconCls: "icon-standard-page-white-put", disabled: !(exp == true || exp.current == true), handler: function () {
            return t.treegrid("exportExcel", false);
        } };
        var m6 = { text: "导出全部", iconCls: "icon-standard-page-white-stack", disabled: !(exp == true || exp.all == true), handler: function () {
            return t.treegrid("exportExcel", true);
        } };
        $.util.merge(m3.children, parseHeaderColumnsShowHideMenu(t, opts, exts, e, field, eventData));
        if (colOpts.filterable) {
            $.util.merge(m4.children, parseHeaderRowsShowHideMenu(t, opts, exts, e, field, eventData));
        }
        $.util.merge(mm, [m1, m2, "-", m3, m4]);
        var expMenu = [m5, m6];
        if (exp) {
            $.array.merge(mm, "-", typeof exp == "object" && !exp.submenu ? expMenu : { text: "导出数据", iconCls: "icon-standard-page-save", children: expMenu });
        }
        return mm;
    };
    ...
    function parseHeaderColumnsShowHideMenu(t, opts, exts, e, field, eventData) {
        return $.array.map(exts.fieldOptions, function (val) {
            var handler = function (e, field, eventData, t, item, menu) {
                if (val.field == opts.treeField) {
                    $.messager.show("树节点列不能被隐藏。");
                    return;
                }
                var m = $(this),
                ...
                count = $.array.sum(exts.fieldOptions, function (val) {
                    return val.hidden ? 0 : 1;
                });
                var len = exts.fields ? exts.fields.length : 0;
                menu.menu("setIcon", {
                ...
                var mm = m.parent().find(".menu-item:gt(1)").filter(function () {
                    return $(".tree-checkbox1", this).length ? true : false;
                });
                mm.each(function () {
                    menu.menu(mm.length > 1 ? "enableItem" : "disableItem", this);
                });
            };
            return {
        ...
        var hasMore = distinctVals.length >= 15,
            ...
            items = $.array.map(data, function (val) {
                var filterRows = $.array.filter(rows, function (value) {
                    ...
                    filterData = t.treegrid("getHiddenRows"),
                    hiddenLength = $.array.sum(filterData, function (value) {
                ...
                var handler = function (e, field, eventData, t, item, menu) {
                    ...
                    t.treegrid(hiddenLength ? "showRows" : "hideRows", $.array.map(filterRows, function (val) {
                        return val[opts.idField];
                    }));
                ...
                $("<div></div>").append("<div>列：" + title + "，共" + distinctVals.length + "项</div><hr />").css({
                    ...
                    $.each(distinctVals, function (index, text) {
                        var id = "itemCheckbox_" + $.util.guid("N"),
                            checked = !$.array.some(exts.filterData, function (val) {
                            ...
                            handler = function () {
                                var filterRows = $.array.filter(rows, function (val) {
                                    ...
                                    hiddenLength = $.array.sum(exts.filterData, function (val) {
                                        return val[field] == text ? 1 : 0;
                                    });
                                t.treegrid(hiddenLength ? "showRows" : "hideRows", $.array.map(filterRows, function (val) {
                                    return val[opts.idField];
    ...
    function parseRowBaseContextMenuItems(t, opts, exts, e, row, eventData) {
        ...
        if (typeof paging == "object") {
            paging = $.extend({ disabled: false, submenu: true }, paging);
        }
        ...
        var m2 = {
            text: "首页", iconCls: "pagination-first", disabled: function () {
        ...
        var m3 = {
            text: "上一页", iconCls: "pagination-prev", disabled: function () {
                return !opts.pagination || eventData.page <= 1;
            },
            handler: function () {
                if (eventData.page > 1) {
        ...
        var m4 = {
            text: "下一页", iconCls: "pagination-next", disabled: function () {
                return !opts.pagination || eventData.page >= eventData.pageCount;
            },
            handler: function () {
        ...
        var m5 = {
            text: "末页", iconCls: "pagination-last", disabled: function () {
                return !opts.pagination || eventData.page >= eventData.pageCount;
            },
            handler: function () {
                if (eventData.page < eventData.pageCount) {
        ...
        var m6 = { text: "展开当前所有", iconCls: "icon-metro-expand", disabled: !(toggle == true || toggle.expandAll == true), handler: function () {
            t.treegrid("expandAll", id);
        } };
        var m7 = { text: "展开当前", iconCls: "icon-metro-expand2", disabled: !(toggle == true || toggle.expand == true), handler: function () {
            t.treegrid("expand", id);
        } };
        var m8 = { text: "折叠当前", iconCls: "icon-metro-contract2", disabled: !(toggle == true || toggle.collapse == true), handler: function () {
            t.treegrid("collapse", id);
        } };
        var m9 = { text: "折叠当前所有", iconCls: "icon-metro-contract", disabled: !(toggle == true || toggle.collapseAll == true), handler: function () {
            t.treegrid("collapseAll", id);
        } };
        var m10 = { text: "上移一级", iconCls: "icon-standard-arrow-up", disabled: !(move == true || move.upLevel == true), handler: function () {
            t.treegrid("shiftRow", { point: "upLevel", id: id });
        } };
        var m11 = { text: "上移", iconCls: "icon-standard-up", disabled: !(move == true || move.up == true), handler: function () {
            t.treegrid("shiftRow", { point: "up", id: id });
        } };
        var m12 = { text: "下移", iconCls: "icon-standard-down", disabled: !(move == true || move.down == true), handler: function () {
            t.treegrid("shiftRow", { point: "down", id: id });
        } };
        var m13 = { text: "下移一级", iconCls: "icon-standard-arrow-down", disabled: !(move == true || move.downLevel == true), handler: function () {
            t.treegrid("shiftRow", { point: "downLevel", id: id });
        } };
        var m14 = { text: "导出当前页", iconCls: "icon-standard-page-white-put", disabled: !(exp == true || exp.current == true), handler: function () {
            return t.treegrid("exportExcel", false);
        } };
        var m15 = { text: "导出全部", iconCls: "icon-standard-page-white-stack", disabled: !(exp == true || exp.all == true), handler: function () {
            return t.treegrid("exportExcel", true);
        } };
        mm.push(m1);
        ...
        if (t.treegrid("isRoot", id)) {
            $.array.insertRange(toggleMenu, 0, [
                { text: "展开所有", iconCls: "icon-standard-arrow-out", handler: function () {
                    t.treegrid("expandAll");
                } },
                { text: "折叠所有", iconCls: "icon-standard-arrow-in", handler: function () {
        ...
        if (paging) {
            $.array.merge(mm, "-", typeof paging == "object" && !paging.submenu ? pagingMenu : { text: "翻页", iconCls: "", disabled: !(paging == true || !paging.disabled), children: pagingMenu });
        }
        if (toggle) {
            $.array.merge(mm, "-", typeof toggle == "object" && !toggle.submenu ? toggleMenu : { text: "展开/折叠", iconCls: "", disabled: !toggle, children: toggleMenu });
        }
        if (move) {
            $.array.merge(mm, "-", typeof move == "object" && !move.submenu ? moveMenu : { text: "上/下移动", iconCls: "", disabled: !move, children: moveMenu });
        }
        if (exp) {
            $.array.merge(mm, "-", typeof exp == "object" && !exp.submenu ? expMenu : { text: "导出数据", iconCls: "icon-standard-page-save", disabled: !exp, children: expMenu });
        }
        return mm;
    };
    ...
    var parseRowContextMenuMap = $.fn.treegrid.extensions.parseRowContextMenuMap = function (e, row, eventData, contextMenu, t) {
        return $.array.map(contextMenu, function (value, index) {
            ...
            ret.onclick = $.isFunction(value.onclick) ? function (e, item, menu) {
                value.onclick.call(this, e, row, eventData, t, item, menu);
            } : value.onclick;
            ret.handler = $.isFunction(value.handler) ? function (e, item, menu) {
                value.handler.call(this, e, row, eventData, t, item, menu);
            } : value.handler;
            if (ret.children && ret.children.length) {
                ret.children = parseRowContextMenuMap(e, row, eventData, ret.children, t);
            }
            return ret;
        });
    };
    ...
    function initAutoEditingEvent(t, opts, exts) {
        ...
        opts[opts.autoEditingEvent] = function (row) {
            if ($.isFunction(exts[opts.autoEditingEvent])) {
                exts[opts.autoEditingEvent].apply(this, arguments);
            }
            if (opts.autoEditing) {
                t.treegrid("beginEdit", row[opts.idField]);
            }
        }
    }
        ...
        $(opts.finishEditLocale).click(function (e) {
            if (opts.finishEditOnBlur && $.data(t[0], "treegrid")) {
                ...
                if (!$.contains(body[0], e.target)) {
                    $.each(rows, function (ii, id) {
                        t.treegrid(opts.finishEditMethod, id);
                    });
                }
            }
        });
    ...
    var initTreeGridExtensions = $.fn.treegrid.extensions.initTreeGridExtensions = function (t, opts, exts) {
        ...
        opts.onClickRow = function (row) {
            if ($.isFunction(exts.onClickRowBak)) {
                exts.onClickRowBak.apply(this, arguments);
            }
        ...
        opts.onCheck = function (row) {
            if ($.isFunction(exts.onCheckBak)) {
                exts.onCheckBak.apply(this, arguments);
            }
            if (opts.cascadeCheck) {
                ...
                while (parent) {
                    ...
                    if (!$.array.some(children, function (val) {
                        return !$.array.contains(checked, val);
                    })) {
                        if (!t.treegrid("isChecked", parent[idField])) {
                            t.treegrid("check", parent[idField]);
                        }
                    }
                    parent = t.treegrid("getParent", parent[idField]);
                }
                $.each(t.treegrid("getChildren", id), function (i, n) {
                    if (!t.treegrid("isChecked", n[idField])) {
                        t.treegrid("check", n[idField]);
                    }
                });
            }
        };
        ...
        opts.onUncheck = function (row) {
            if ($.isFunction(exts.onUncheckBak)) {
                exts.onUncheckBak.apply(this, arguments);
            }
            if (opts.cascadeCheck) {
                if (opts.checkOnSelect && opts.singleSelect) {
                    return;
                ...
                while (parent) {
                    ...
                    if (!$.array.some(children, function (val) {
                        ...
                        if (t.treegrid("isChecked", parent[idField])) {
                            t.treegrid("uncheck", parent[idField]);
                        }
                    }
                    parent = t.treegrid("getParent", parent[idField]);
                }
    ...
    var initColumnRowTooltip = function (t, opts, id, row, tr) {
        ...
        if (opts.rowTooltip) {
            ...
            tr.each(function () {
                $.easyui.tooltip.init(this, { onShow: onShow });
            });
        } else {
        ...
        function buildText(row) {
            ...
            $.each(cols, function (i, colOpts) {
                if (!colOpts || !colOpts.field || !colOpts.title) {
                    return;
                }
    ...
    var initSingleEditing = function (t, opts, id) {
        ...
        if (opts.singleEditing) {
            t.treegrid("endEdit", exts.lastEditingId);
        }
    ...
    var initExtensions = $.fn.treegrid.extensions.initExtensions = function (t, opts) {
        ...
        exts.fields = $.array.filter(fields, function (val) {
            return t.treegrid("getColumnOption", val).title ? true : false;
        });
        exts.fieldOptions = $.array.map(exts.fields, function (val) {
            return t.treegrid("getColumnOption", val);
        });
        ...
        function initOffset() {
            t.treegrid("setOffset", opts.offset);
        };
        ...
        function initAutoEditing() {
            initAutoEditingEvent(t, opts, exts);
        };
        function initFinishEdit() {
            initFinishEditEvent(t, opts, exts);
        };
        function initTreeExtensions() {
            initTreeGridExtensions(t, opts, exts);
        };
    ...
    var loadFilter = $.fn.treegrid.extensions.loadFilter = function (data, parent) {
        if ($.isFunction(_loadFilter)) {
            data = _loadFilter.apply(this, arguments);
        }
        ...
        if (parent != null && parent != undefined) {
            return isArray ? rows : { total: rows.length, rows: rows };
        }
    ...
    var onExpand = $.fn.treegrid.extensions.onExpand = function (row) {
        if ($.isFunction(_onExpand)) {
            _onExpand.apply(this, arguments);
        }
        var t = $(this), opts = t.treegrid("options");
        if (opts.onlyNodeExpand) {
            var rows = t.treegrid("getNears", row[opts.idField]), animate = opts.animate
            opts.animate = false;
            $.each($.array.filter(rows, function (val) {
                return val[opts.idField] != row[opts.idField] && val.state == "open";
            }), function () {
    ...
    var methods = $.fn.treegrid.extensions.methods = {
        ...
        isSelected: function (jq, id) {
            return isSelected(jq[0], id);
        },
        ...
        isEditing: function (jq, id) {
            return isEditing(jq[0], id);
        },
        ...
        getEditingNodeId: function (jq) {
            return getEditingNodeId(jq[0]);
        },
        ...
        getEditingNodeIds: function (jq) {
            return getEditingNodeIds(jq[0]);
        },
        ...
        isRoot: function (jq, id) {
            return isRootNode(jq[0], id);
        },
        ...
        compare: function (jq, param) {
            return compareNode(jq[0], param);
        },
        ...
        isSibling: function (jq, param) {
            return isSibling(jq[0], param);
        },
        ...
        nextRow: function (jq, id) {
            return getNextRow(jq[0], id);
        },
        ...
        prevRow: function (jq, id) {
            return getPrevRow(jq[0], id);
        },
        ...
        getNears: function (jq, id) {
            return getNears(jq[0], id);
        },
        ...
        getNearChildren: function (jq, id) {
            return getNearChildren(jq[0], id);
        },
        ...
        getRows: function (jq, cascade) {
            return getRows(jq[0], cascade);
        },
        ...
        getColumnData: function (jq, param) {
            return getColumnData(jq[0], param);
        },
        ...
        //          否则返回一个空的 jQuery 对象。
        getRowDom: function (jq, param) {
            return getRowDom(jq[0], param);
        },

        //  获取当前 easyui-treegrid 中当前页指定 id 的节点数据对象；同 find 方法。
        getRowData: function (jq, id) {
            return getNode(jq[0], id);
        },
        ...
        getDistinctRows: function (jq, param) {
            return getDistinctRows(jq[0], param);
        },
        ...
        //      如果传入的列名不存在，则返回一个长度为 0 的数组对象.
        getDistinctColumnData: function (jq, param) {
            return getDistinctColumnData(jq[0], param);
        },
        ...
        //      如果 cascade 为 false，则仅返回所有根节点数据，同 getRoots 方法；
        //      如果 easyui-treegrid 的当前页没有数据，则返回一个长度为 0 的数组。
        getHiddenRows: function (jq, cascade) {
            return getHiddenRows(jq[0], cascade);
        },
        ...
        //      如果 cascade 为 false，则仅返回所有根节点数据，同 getRoots 方法；
        //      如果 easyui-treegrid 的当前页没有数据，则返回一个长度为 0 的数组。
        getVisibleRows: function (jq, cascade) {
            return getVisibleRows(jq[0], cascade);
        },
