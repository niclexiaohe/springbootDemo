...
(function ($, undefined) {
    ...
    function getLevel(treeTarget, target) {
        ...
        if (!t[0] || !node[0] || !node.is(".tree-node[node-id]") || !$.contains(t[0], node[0])) {
            return 0;
        }
    ...
    function moveNode(treeTarget, param) {
        ...
        if ($.isFunction(opts.onBeforeDrop) && opts.onBeforeDrop.call(treeTarget, param.target, param.source, param.point) == false) {
            return;
        }
        if (t.tree("isParent", { target1: param.source, target2: param.target })) {
            return;
        }
        ...
        if (node && $.isFunction(opts.onDrop)) {
            opts.onDrop.call(treeTarget, param.target, param.source, param.point);
        }
    };

    function shiftNode(treeTarget, param) {
        if (!param || !param.target || !param.point || !$.array.contains(["up", "upLevel", "down", "downLevel"], param.point)) {
            return;
        }
        param.target = $(param.target)[0];
        ...
        switch (param.point) {
            case "up":
                targetNode = t.tree("prev", source);
                break;
            case "upLevel":
                targetNode = t.tree("getParent", source);
                break;
            case "down":
                targetNode = t.tree("next", source);
                break;
            case "downLevel":
                targetNode = t.tree("prev", source);
                break;
    ...
    function initBindDblClickEvent(t, opts, exts) {
        if (!$.array.likeArray(opts.contextMenu) || $.util.isString(opts.contextMenu) || !opts.contextMenu.length || !opts.autoBindDblClick) {
            return;
        }
        opts.onDblClickBak = opts.onDblClick;
        opts.onDblClick = function (node) {
            if ($.isFunction(exts.onDblClickBak)) {
                exts.onDblClickBak.apply(this, arguments);
            }
    ...
    function parseContextMenuItems(t, opts, e, node) {
        ...
        var mExpandAll = { text: "展开当前所有", iconCls: "icon-metro-expand", disabled: !(toggle == true || toggle.expandAll == true), handler: function () {
            t.tree("expandAll", node.target);
        } };
        var mExpand = { text: "展开当前", iconCls: "icon-metro-expand2", disabled: !(toggle == true || toggle.expand == true), handler: function () {
            t.tree("expand", node.target);
        } };
        var mCollapse = { text: "折叠当前", iconCls: "icon-metro-contract2", disabled: !(toggle == true || toggle.collapse == true), handler: function () {
            t.tree("collapse", node.target);
        } };
        var mCollapseAll = { text: "折叠当前所有", iconCls: "icon-metro-contract", disabled: !(toggle == true || toggle.collapseAll == true), handler: function () {
            t.tree("collapseAll", node.target);
        } };

        var mUpLevel = { text: "上移一级", iconCls: "icon-standard-arrow-up", disabled: !(move == true || move.upLevel == true), handler: function () {
            t.tree("shift", { point: "upLevel", target: node.target });
        } };
        var mUp = { text: "上移", iconCls: "icon-standard-up", disabled: !(move == true || move.up == true), handler: function () {
            t.tree("shift", { point: "up", target: node.target });
        } };
        var mDown = { text: "下移", iconCls: "icon-standard-down", disabled: !(move == true || move.down == true), handler: function () {
            t.tree("shift", { point: "down", target: node.target });
        } };
        var mDownLevel = { text: "下移一级", iconCls: "icon-standard-arrow-down", disabled: !(move == true || move.downLevel == true), handler: function () {
            t.tree("shift", { point: "downLevel", target: node.target });
        } };

        var mOpts = {
            text: "显示 Option", iconCls: "icon-standard-application-form", disabled: !opts.showOption, children: [
                { text: "树控件 Option", iconCls: "icon-hamburg-category", handler: function () {
                    t.tree("showOption");
                } },
                { text: "该节点 Option", iconCls: "tree-icon tree-file", handler: function () {
                    t.tree("showOption", node.target);
                } }
            ]
        };
        ...
        if ($.array.likeArray(opts.contextMenu)) {
            //如果展开或移动节点为true便在自定义按钮后加横线
            if ((opts.toggleMenu || opts.moveMenu) && !opts.showOption) {
                $.array.merge(menus, opts.contextMenu, "-");
            } else {
                $.array.merge(menus, opts.contextMenu);
            }
        }
        //如果有showOption
        if (opts.showOption) {
            if (opts.toggleMenu || opts.moveMenu) {
                $.array.merge(menus, mOpts, "-");
            } else {
                $.array.merge(menus, mOpts);
            }
        }
        ...
        if (opts.toggleMenu) {
            $.array.merge(menus, typeof toggle == "object" && !toggle.submenu
                ? $.array.merge(toggleMenu, "-") : [
                { text: "展开/折叠", iconCls: "", disabled: !toggle, children: toggleMenu }
            ]);
        }
        //移动
        if (opts.moveMenu) {
    ...
    function parseContextMenuMap(e, node, menus, t) {
        return $.array.map(menus, function (value) {
            ...
            ret.onclick = $.isFunction(value.onclick) ? function (e, item, menu) {
                value.onclick.call(this, e, node, t, item, menu);
            } : value.onclick;
            ret.handler = $.isFunction(value.handler) ? function (e, item, menu) {
                value.handler.call(this, e, node, t, item, menu);
            } : value.handler;
            if (ret.children && ret.children.length) {
                ret.children = parseContextMenuMap(e, node, ret.children, t);
            }
            return ret;
        });
    };
    ...
    var onExpand = $.fn.tree.extensions.onExpand = function (node) {
        ...
        if (opts.onlyNodeExpand) {
            ...
            opts.animate = false;
            $.each($.array.filter(nodes, function (val) {
    ...
    $.fn.tree.extensions.dataPlainConverter = function (data, opts) {
        ...
        if (opts.dataPlain) {
            var roots = $.array.filter(data, function (val) {
                if (val[parentField] == null || val[parentField] == undefined) {
                    return true;
                }
    ...
    var methods = $.fn.tree.extensions.methods = {
        ...
        isRoot: function (jq, target) {
            return isRootNode(jq[0], target);
        },
        ...
        showOption: function (jq, target) {
            return jq.each(function () {
                showOption(this, target);
            });
        },
        ...
        //  返回值：返回表示当前 easyui-tree 组件的 jQuery 对象。
        shift: function (jq, param) {
            return jq.each(function () {
                shiftNode(this, param);
            });
        },
        ...
        next: function (jq, target) {
            return getNextNode(jq[0], target);
        },
        ...
        prev: function (jq, target) {
            return getPrevNode(jq[0], target);
        },
        ...
        getNears: function (jq, target) {
            return getNears(jq[0], target);
        },
        ...
        getNearChildren: function (jq, target) {
            return getNearChildren(jq[0], target);
        },
