...
(function (window, $, undefined) {
    ...
    coreUtil.getRequest = function () {
        var search = location.search;//返回一个URL的查询部分
        if (search.substr(0, 1) == "?") { search = search.substr(1, search.length - 1); }
        var result = [];
        if (search.length > 0) {
            var params = search.split("&");//以&进行分割字符串
            for (var i = 0; i < params.length; i++) {
                var param = params[i];
                var pos = param.indexOf("=");//获得=符号的索引值
                var name = param.substring(0, pos);//拿到url的查询部分的每个参数名
                var value = param.substr(pos + 1);//拿到url的查询部分的每个参数名对应的参数值
                result.push({ name: name, value: value });//将参数名和参数值组合成json对象，并放入数组中
                result[name] = value;
            }
        }
        return result;
    ...
    coreString.contains = function (str, val) {
        str = coreString.isNullOrEmpty(str) ? "" : String(str);
        //根据判断子字符串val在String(str)字符串中的索引位置来判断是否包含子字符串！！
    ...
    /*******************************************************************
                          add notes by hechuan at 20170329PM
    1、本项目中此处obj={account: $("#account").val(),password:$("#password").val()}
    2、假设，我们登陆时输入的账户名为：admin，密码为:1；
    3、 则此处的obj={account: "admin",password:"1"};
    4、经过下面方法转换后得到返回的val值为：{account: "admin",password:"1"}
    ********************************************************************/
    ...
    coreUtil.requestAjaxData = function (url, args) {
        var val = null;
        //此处的args是json数据对象：{account: "admin",password:"1"}
        //将args转换成json数据格式对象
        args = coreUtil.parseMapFunction(args);
        //ajax请求
        $.ajax({
            //url表示请求到远程（服务器）的地址；
            //type："POST"：表示请求到远程地址时传递数据的方式为“post”方式；
            //data: args：表示请求到远程地址时传递的参数（数据）（这个args数据为Json格式的数据对象）；
            // async设置成true表示是异步操作，false表示同步操作；
            url: url, type: "POST", data: args, async: false,
            /*自动判断 (xml 或 html)) 请求失败时调用此函数。有以下三个参数：XMLHttpRequest 对象、错误信息、（可选）捕获的异常对象。
            *如果发生了错误，错误信息（第二个参数）除了得到null之外，还可能是"timeout", "error", "notmodified" 和 "parsererror"。
            */
            error: function (XMLHttpRequest, textStatus, errorThrown) {
                $.messager.progress("close");
                if ($.easyui.messager != $.messager) {
                    $.easyui.messager.progress("close");
                }
                var msg;
                if(XMLHttpRequest.status == 999 || XMLHttpRequest.status == 998){
                    msg = "当前登陆已失效,请重新登陆.<br>系统将在10秒后自动跳转登陆页面或按F5刷新重新登陆."
                    //10s
                    setTimeout(function(){
                        window.webSocketMain.logout();
                        top.location.href = "indexAction.do?method=initSystemManager";
                    },10000);
                }else{
                    //msg = (XMLHttpRequest && !$.string.isNullOrWhiteSpace(XMLHttpRequest.responseText) ?
                ...
                win.window("resize", { width:600 });
                win.window("center");
            },
            /*请求成功后的回调函数。参数：由服务器返回，并根据dataType参数进行处理后的数据；描述状态的字符串。
             *还有 jqXHR（在jQuery 1.4.x的中，XMLHttpRequest） 对象 。
             */
            //success: function (data, textStatus, jqXHR)中的data不同于上面的data！上面的data是作为参数传递到远程服务器请求地址，下面的data是请求远程地址后返回的值！！！
            //请求成功后，将服务器传递过来的值（response中的值）赋值给val
            //经过上面ajax调用远程服务器的处理后返回的值是json对象（因为对应的controller中进行了json处理后返回）！！
            success: function (data, textStatus, jqXHR) {
                val = data;//data是返回的一个json对象字符串
                //alert(val);  //test by hehcuan at 20170329PM，通过这条语句测试可以看到执行上面ajax请求后返回的data值！手机上有照片！
            }
        });
        return val;
    };
